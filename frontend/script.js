const connectWalleMsg = document.querySelector('#connectWalletMessage');
const connectWalleBtn = document.querySelector('#connectWallet');
...
const contractABI = [
  ...
  const getResult = async function(){
    result.computedStyleMap.display = "flex";
  ...
  const refreshPage = function() {
    setInterval(async()=>{
        ...
        if (time > 0) {
            ...
            timerMessage.textContent = "Either there's no election or the election already ended";
  ...
  const startElection = async function () {
    ...
    showResultContainer.styke.display = "none";
  ...
  const getAccount = async function () {
    ...
    connectWalleBtn.textContent = signer._address.slice(0,10)+ "...";
    connectWalleMsg.textContent = "You are currently connected";
    connectWalleBtn.disabled = true;
  ...
  connectWalleBtn.addEventListener('click',getAccount);
